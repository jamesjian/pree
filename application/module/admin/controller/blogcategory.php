...
class Blogcategory extends Admin {
    ...
    public function search() {
        if (isset($_POST['search']) && trim($_POST['search']) != '') {
            $link = ADMIN_HTML_ROOT . 'blogcategory/retrieve/1/title/ASC/' . trim($_POST['search']);
        } else {
            $link = ADMIN_HTML_ROOT . 'blogcategory/retrieve/1/title/ASC/';
        }
    ...
    public function retrieve() {
        ...
        $cat_list = Model_Blogcategory::get_cats_by_page_num($where, $current_page, $order_by, $direction);
        $num_of_records = Model_Blogcategory::get_num_of_cats($where);
